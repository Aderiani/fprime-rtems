...
static_assert(DeframerCfg::POLL_BUFFER_SIZE > 0, "poll buffer size must be greater than zero");
static_assert(DeframerCfg::RING_BUFFER_SIZE > 0, "ring buffer size must be greater than zero");
...
void Deframer ::cmdResponseIn_handler(FwIndexType portNum,
                                      FwOpcodeType opcode,
                                      U32 cmdSeq,
...
void Deframer ::framedIn_handler(const FwIndexType portNum, Fw::Buffer& recvBuffer, const Drv::RecvStatus& recvStatus) {
...
void Deframer ::schedIn_handler(const FwIndexType portNum, U32 context) {
    // Check for data
...
void Deframer ::route(Fw::Buffer& packetBuffer) {
    ...
    if (status == Fw::FW_SERIALIZE_OK) {
        ...
        switch (packetType) {
            ...
            case Fw::ComPacket::FW_PACKET_LOG: {
                // Allocate a com buffer for the event
                Fw::ComBuffer com;
                // Copy the packet data into the com buffer
                status = com.setBuff(packetData, packetSize);
                if (status == Fw::FW_SERIALIZE_OK) {
                    // Send to the event output port (if you have one)
                    // Or send through the same comOut port
                    comOut_out(0, com, 0);
                }
                break;
            }
            default:
                Fw::Logger::log("[WARNING] Unhandled packet type: %d\n", packetType);
                break;
        }
    } else {
...
void Deframer ::processRing() {
    ...
    // The protocol status
    DeframingProtocol::DeframingStatus status = DeframingProtocol::DEFRAMING_STATUS_SUCCESS;
    ...
    for (FwSizeType i = 0; i < ringCapacity; i++) {
        ...
        if (status == DeframingProtocol::DEFRAMING_STATUS_SUCCESS) {
            ...
            // to a non-zero value
            FW_ASSERT(needed != 0);
            FW_ASSERT(needed <= remaining, static_cast<FwAssertArgType>(needed),
                      static_cast<FwAssertArgType>(remaining));
            m_inRing.rotate(needed);
            FW_ASSERT(m_inRing.get_allocated_size() == remaining - needed,
                      static_cast<FwAssertArgType>(m_inRing.get_allocated_size()),
                      static_cast<FwAssertArgType>(remaining), static_cast<FwAssertArgType>(needed));
        }
        // More data needed
