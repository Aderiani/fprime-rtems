...
Fw::Buffer BufferManagerComponentImpl ::bufferGetCallee_handler(const FwIndexType portNum, U32 size) {
    // make sure component has been set up
    FW_ASSERT(this->m_setup);
    FW_ASSERT(m_buffers);
...
void BufferManagerComponentImpl::setup(NATIVE_UINT_TYPE mgrId,       //!< manager ID
    ...
    for (NATIVE_UINT_TYPE bin = 0; bin < BUFFERMGR_MAX_NUM_BINS; bin++) {
        if (this->m_bufferBins.bins[bin].numBuffers) {
            memorySize += (this->m_bufferBins.bins[bin].bufferSize *
                          ...
                          (static_cast<NATIVE_UINT_TYPE>(sizeof(AllocatedBuffer)) *
    ...
    // Ensure allocation is 8-byte aligned for SPARC architecture
