...
def disable_workflow(workflow_path):
    ...
    if 'workflow_dispatch:' in content and 'push:' not in content.replace('# push:', ''):
    ...
    for line in lines:
        if line.strip() == 'on:':
            ...
            new_lines.append('  workflow_dispatch:  # Manual trigger only during GR740 development')
        elif in_on_section:
            if line.strip() and not line.startswith(' '):
                in_on_section = False
                new_lines.append(line)
            elif 'push:' in line or 'pull_request:' in line or 'pull_request_target:' in line:
                new_lines.append('  # ' + line.strip())
            elif line.strip().startswith('branches:') or line.strip().startswith('paths'):
                new_lines.append('  # ' + line.strip())
...
def enable_workflow(workflow_path):
    ...
    content = content.replace('  workflow_dispatch:  # Manual trigger only during GR740 development\n', '')
    ...
    for line in lines:
        if line.strip().startswith('# push:') or line.strip().startswith('# pull_request'):
            new_lines.append(line.replace('# ', '', 1))
        elif line.strip().startswith('#   - ') or line.strip().startswith('# branches:'):
            new_lines.append(line.replace('# ', '', 1))
        else:
            new_lines.append(line)
...
def main():
    parser = argparse.ArgumentParser(description='Manage GitHub workflows for GR740 development')
    parser.add_argument('action', choices=['disable', 'enable', 'list', 'disable-all', 'enable-all'],
