...
Console::Console()
    ...
    printf("m_delegate address: %p\n", static_cast<void*>(&m_delegate));
    m_delegate.writeMessage("Test", 4);
    printf("Constructor exiting, this = %p\n", static_cast<void*>(this));
    U32 sp;                                   // Declare sp as an unsigned 32-bit integer
    asm volatile("mov %%sp, %0" : "=r"(sp));  // Get stack pointer
...
Console& Console::getSingleton() {
    static Console* s_singleton = nullptr;
    if (!s_singleton) {
        s_singleton = new Console();
        printf("Singleton created at %p\n", static_cast<void*>(s_singleton));
        fflush(stdout);
        Fw::Logger::registerLogger(s_singleton);
    }
    printf("Singleton returning at %p\n", static_cast<void*>(s_singleton));
    fflush(stdout);
    return *s_singleton;
}
}  // namespace Os
