...
class Led : public LedComponentBase {
  public:
    ...
    alignas(4) Fw::On m_state = Fw::On::OFF;  //! Keeps track if LED is on or off
    alignas(8) U64 m_transitions = 0;         //! The number of on/off transitions that have occurred
                                   //! from FSW boot up
    alignas(4) U32 m_toggleCounter = 0;       //! Keeps track of how many ticks the LED has been on for
    alignas(4) bool m_blinking = false;       //! Flag: if true then LED blinking will occur else
